...
class App extends Component {
  state = {
    updatedImagesList: imagesList,
    updatedId: 'FRUIT',
    uniqueUrl: imagesList[0].imageUrl,
    score: 0,
    totalScore: 0,
    time: 60,
    isTimeRunning: true,
  ...
  clearIntervalTime = () => clearInterval(this.timerId)
  ...
  onChangeImage = imageUrl => {
    ...
    this.setState({uniqueUrl: imagesList[RandomNumber].imageUrl})
    const {uniqueUrl, time} = this.state
    if (imageUrl === uniqueUrl && time !== 0) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        totalScore: prevState.score + 1,
      }))
    } else {
      this.setState({
        score: 0,
  ...
  render() {
    ...
    const updatedImages = updatedImagesList.filter(
      each => each.category === updatedId,
    )
    return (
        ...
        {isDisplayScore ? (
          <div className="card-container">
            <div className="score-container">
              ...
              <p className="heading">{totalScore}</p>
              <button
              ...
              >
                <img
                  ...
                  className="reset"
                />
                <p className="paragraph">PLAY AGAIN</p>
              </button>
            </div>
          </div>
        ) : (
          <div className="page-container">
            <img src={uniqueUrl} alt="match" className="main-image" />
